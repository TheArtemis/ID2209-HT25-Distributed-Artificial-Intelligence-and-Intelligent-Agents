...
### 3.5 Landing Pad
- A point of interest at (50, 200).
- Used as the destination for retirement (agents move here and despawn).

---

## 4. Agent Types
All agents share the same base implementation (`Human`) and differ by role-specific beliefs/plans and trade effects.

### 4.1 Shared State (Human)
- **Identity:**
  - `role`: the true role.
  - `presented_role`: what others “see”. Parasites use this to disguise.
- **Biological variables:** `oxygen_level`, `energy_level`, `health_level`.
- **Learning variables:**
  - `Q`: maps partner-id → `[Q(TRADE), Q(IGNORE)]`.
  - `trust_memory`: maps partner-id → trust score in [-1, 1].
  - `happiness`: cumulative reward signal from trades.
- **Personality / learning traits:**
  - `sociability` (learning rate / alpha)
  - `patience` (defined but not currently used)
  - `curiosity` (exploration rate / epsilon)

### 4.2 Engineer
- Repairs the oxygen generator when broken (BDI desire + plan).
- In trades:
  - If generator is broken, can repair it.
  - Otherwise provides oxygen (+10) to the partner.

### 4.3 Medic
- Operates a queue-based healing routine at the Med-Bay.
- Healing is performed **at the Med-Bay** (not through trade).

### 4.4 Scavenger
- Randomly decides to perform mining missions.
- In trades:
  - If it has `raw_amount > 0`, it can give 1 raw unit and increase partner energy.
  - If it has no raw, it still provides limited help at an energy cost.

### 4.5 Commander
- Monitors dust storms and warns agents in the wasteland using FIPA.
- In trades:
  - Provides both oxygen (+10) and energy (+10) to the partner.

### 4.6 Parasite (adversary)
- Disguises its `presented_role` as one of: Engineer, Medic, Scavenger, Commander.
- In trades:
  - If partner has raw resources, steals 1.
  - Otherwise drains partner energy (-10) and gains energy (+10).
  - Never provides a beneficial return.

---

## 5. BDI (Challenge 1)
Agents use `simple_bdi` control.

### 5.1 Core beliefs (Human)
- `suffocating_belief` when oxygen < 20%.
- `starving_belief` when energy < 20%.
- `injured_belief` when health < 50%.
- `should_retire_belief` when ETA ≥ 2000.
- `storm_warning_belief` when a FIPA message indicates “Return to Base”.
- `want_to_trade_belief` when motivated to trade (see “Trading Motivation”).

### 5.2 Role beliefs
- Engineer: `generator_broken_belief`.
- Medic: `patients_waiting_belief`.
- Scavenger: `mission_time_belief`.

### 5.3 Desires (rule strengths)
- `escape_storm_desire` (200)
- `has_oxygen_desire` (100) — plan currently disabled
- `has_energy_desire` (25) — plan currently disabled
- `be_healthy_desire` (12)
- Engineer: `fix_generator_desire` (9)
- Medic: `heal_patients_desire` (7)
- `retire_desire` (6)
- Scavenger: `mine_desire` (5)
- `wander_desire` (default)

### 5.4 Plans
- `escape_storm`: move to Habitat Dome.
- `get_health`: move to Med-Bay, join queue, get healed by a Medic.
- `do_retire`: move to Landing Pad and despawn.
- `wander_around`: roam within the Habitat Dome.
- Role-specific plans: Engineer repairs generator; Medic heals queued patients; Scavenger mines.

---

## 6. Interactions & Trade (Meeting Places)

### 6.1 Where trades happen
Trading and learning only happen when agents are inside:
- `CommonArea` OR
- `RecreationArea`

### 6.2 Trading motivation
When inside the dome and not storm-warned, an agent may set `want_to_trade_belief` with probability `0.5 + curiosity`. This triggers the desire to move to the nearest trading area.

### 6.3 Partner selection constraints
- Agents can trade only if `trade_cooldown = 0`.
- Partners are sampled from all agents within `meet_distance` (default 300).
- After each decision, a cooldown of `trade_cooldown_max = 5` is applied.

### 6.4 Trade effects by role
Trade behavior is role-dependent (summarized above in “Agent Types”). This is what creates distinct interaction rules across the 5 agent types.

---

## 7. Reinforcement Learning / Trust (Challenge 2)

Each agent learns per partner id (`"id:" + partner.name`).

### 7.1 State and actions
- **State:** `s = "id:" + partner.name`
- **Actions:** `TRADE` or `IGNORE`
- **Exploration:** with probability `curiosity` an agent explores (random action); otherwise selects action with higher Q value.

### 7.2 Reward function
- Mutual beneficial trade: +20
- Trading when the partner does not reciprocate (typical outcome when interacting with Parasites): -1
- Parasite vs Parasite trade is treated as strongly negative: -20
- Ignore: 0

### 7.3 Q update and trust mapping
- Q update (bandit-style):
  - `Q[s][a] <- Q[s][a] + sociability * (reward - Q[s][a])`
- Trust:
  - `trust = clamp((Q[s][TRADE] - Q[s][IGNORE]) / 10, -1, 1)`
  - Negative trust is treated as a “parasite prediction” in the global detection metrics.

---

## 8. Communication (FIPA)

### 8.1 Sender
- Commander.

### 8.2 Trigger
- If the wasteland is in dust storm and there are agents inside the wasteland region.

### 8.3 Message semantics
- Commander calls `start_conversation` with protocol `"fipa-propose"` and contents `"Return to Base"`.
- Receivers parse mailbox contents; if it contains `"Return to Base"`, they add `storm_warning_belief`, which triggers the `escape_storm` plan.

---

## 9. Biological Dynamics
- Oxygen decreases every tick:
