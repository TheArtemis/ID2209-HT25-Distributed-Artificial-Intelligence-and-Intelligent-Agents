...
\begin{itemize}
  ...
  \item Agents spawn at the habitat dome location, not the landing pad
    \item The variable \texttt{patience} is defined but not currently used in decision-making
\end{itemize}
...
{Note:} In the current implementation, the BDI plans \texttt{get\_oxygen} 
and \texttt{get\_energy} are intentionally disabled to prioritize learning through social interactions.
Oxygen/energy can still change due to environmental drain, role-specific trades (e.g., Engineers/Commanders), and the Med-Bay proximity "immunity" behavior.
...
\begin{itemize}
  ...
  \item \texttt{total\_trades}: Counter of total trade interactions
    \item \texttt{avg\_sociability}: Average learning-rate parameter across agents
...
\begin{itemize}
  \item \texttt{escape\_storm} (200.0) - Highest priority
    \item \texttt{has\_oxygen} (100.0) - High priority (plan currently disabled)
    \item \texttt{has\_energy} (25.0) - Medium priority (plan currently disabled)
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Input:} An agent's oxygen level drops below 20\% threshold.
    \item \textbf{Screenshot:} [PLACEHOLDER: Screenshot showing agent state inspector with an agent having \texttt{oxygen\_level} $<$ 20. Console should show the \texttt{suffocating\_belief} being added and the \texttt{has\_oxygen\_desire} being created.]
    \item \textbf{Interpretation:} The perception reflex correctly detects low oxygen and adds the \texttt{suffocating\_belief}. The BDI system creates the \texttt{has\_oxygen\_desire} with high priority (100.0). In the current implementation, the corresponding plan \texttt{get\_oxygen} is disabled to prioritize learning interactions.
...
\begin{itemize}
  ...
  \item \textbf{Q-Value Update:} $Q(s,a) = Q(s,a) + sociability \times (reward - Q(s,a))$ where $sociability = 0.2$
    \item \textbf{Trust Calculation:} $trust = (Q[TRADE] - Q[IGNORE]) / 10.0$, clamped to $[-1.0, 1.0]$
