...
\begin{itemize}
  ...
  \item The variable \texttt{patience} (discount factor) is defined but not currently used in learning (set to 0.0)
...
Agents actively manage their biological needs through BDI plans. When oxygen or energy levels drop below thresholds, agents navigate to the appropriate facilities (Oxygen Generator or Greenhouse) to replenish resources. Facilities automatically provide replenishment when agents are within proximity.
...
\begin{itemize}
  ...
  \item \textbf{Rewards:}
    \begin{itemize}
      \item Successful mutual trade: +100.0
      \item Scammed by Parasite (victim or parasite): -80.0
      \item Altruistic giving (non-parasite gave but partner didn't): +20.0
      \item Ignore action: +2.0 (small positive reward for avoiding potentially bad interactions)
      \item Neutral: 0.0
    \end{itemize}
  \item \textbf{Q-Value Update:} $Q(s,a) = Q(s,a) + sociability \times (reward - Q(s,a))$ where $sociability = 0.35$ (increased for faster convergence)
    \item \textbf{Trust Calculation:} $trust = (Q[TRADE] - Q[IGNORE]) / 100.0$, clamped to $[-1.0, 1.0]$ (normalized by max expected difference)
...
Agents adapt their learning parameters based on experience. Every 100 cycles, agents adjust their \texttt{curiosity} (exploration rate) based on average trust: if trust is low (indicating poor learning), curiosity increases to encourage more exploration; if trust is high (indicating good learning), curiosity decreases to exploit learned knowledge. Additionally, if \texttt{sociability} drops below 0.15, it gradually recovers to prevent agents from getting stuck in low-learning states.
...
float patience <- 0.0;      // Discount factor (not currently used)
float curiosity <- 0.20;     // Exploration rate (adaptively adjusted)
