...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Screenshot:} [PLACEHOLDER: Screenshot showing agent state inspector with an agent having \texttt{oxygen\_level} $<$ 20. The agent should have state showing it is responding to the suffocating belief (e.g., \texttt{"going\_to\_oxygen"} or similar). Console should show belief additions and desire creations. The agent should be moving toward an oxygen source.]
  ...
  \item \textbf{Screenshot:} [PLACEHOLDER: Screenshot showing agent state inspector with an agent having \texttt{health\_level} $<$ 50. The agent should have state \texttt{"going\_to\_med\_bay"} or \texttt{"waiting\_at\_med\_bay"}. The med-bay should show the agent in its queue (orange color, queue count $>$ 0). Console should show the agent adding itself to the med-bay queue.]
  ...
  \item \textbf{Screenshot:} [PLACEHOLDER: Screenshot showing multiple agents with different states: one \texttt{"escaping\_storm"} (highest priority), one \texttt{"going\_to\_med\_bay"} (medium priority), one \texttt{"going\_to\_oxygen"} (high priority). The agent state inspector should show different agents prioritizing different tasks. Console should show the priority system in action.]
...
\begin{itemize}
  ...
  \item \textbf{Actions:} TRADE or IGNORE (selected via curiosity-greedy: 20\% random exploration, 80\% exploitation)
  \item \textbf{Rewards:}
    \begin{itemize}
    ...
    \end{itemize}
  \item \textbf{Q-Value Update:} $Q(s,a) = Q(s,a) + sociability \times (reward - Q(s,a))$ where $sociability = 0.2$
