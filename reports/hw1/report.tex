...
follow a guest and attack it, either by changing the direction towards it is moving or by reducing/adding thirstiness and hungryness.
The other guests can report him to the InformationCenter which will use the SecurityGuard to kill the BadApple.
...
between the agents. In particular this is the portion of code 
...
One important aspect is that guests know the positionon the InformationCenter by default.
\begin{verbatim}
  ask guests {
            	infoCenter <- center[0];
        	}
...
Finally, when the user reaches a shop wither the hungriness or the thirstiness are solved thanks to this
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{imgs/code/smartGuestRiccardo.png}
  \caption{Code snippet for SmartGuest behavior}
  \label{fig:smartGuestRiccardo}
\end{figure}
...
\subsection*{Challenge 2: Security Guard and Bad Apples}
\subsubsection*{4.1 Explanation}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Victim detects that it is being attacked
...
\end{enumerate}
\subsubsection*{4.1.1 BadApple Attack}
...
\end{figure}
\subsubsection*{4.1.2 Reporting and Security Guard Intervention}
Guest detects when it is being attacked by a BadApple. Upon detection, the victim Guest reports the incident to the InformationCenter, providing details about the BadApple's identity and location.
\subsubsection*{4.1.3 InformationCenter is notified about the attack and informs the SecurityGuard.}
The Information Center then instructs the Security Guard to locate and eliminate the reported BadApple.
\subsubsection*{4.1.4 The SecurityGuard locates and eliminates the BadApple.}
The Security Guard moves towards the BadApple's location and "kills" it upon reaching it, as shown in the figure below.
\begin{figure}[H]
  \centering
