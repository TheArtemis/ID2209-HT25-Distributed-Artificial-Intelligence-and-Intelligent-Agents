...
On the screen you will also see two charts regarding surplus and revenue charts of all auction types.
This part will be explained better inside section Challenge number 2.
...
\begin{verbatim}
    ...
    float auctioned_magic_crystals_price <- 
        rnd(magic_crystals_price * min_inc_coeff, 
            magic_crystals_price * max_inc_coeff);  
    ...
    float baseline_magic_crystals_price <- 
        magic_crystals_price * baseline_min_price_coeff;  
...
The guest is receiving a CFP and processes it thanks to this method. Note that guests can only participate in auctions when they are within range of an auctioneer (within 6 units) or have reached the auctioneer location:
\begin{verbatim}
    reflex receiveCFP when: !empty(cfps) and hunger < hungerThreshold 
                       and thirsty < thirstThreshold 
                       and onTheWayToShop = false and targetShop = nil 
                       and beingAttacked = false 
                       and ((targetAuctioneer != nil 
                             and (location distance_to targetAuctioneer.location) < 6.0) 
                            or currentAction = "at auctioneer")
\end{verbatim}
...
\subsubsection*{Guests Visiting Auctioneers}
Guests can only participate in auctions when they are within range of an auctioneer. While in a wandering state, guests can decide to physically visit auctioneers. This behavior is controlled by a global probability parameter that determines the likelihood of a guest choosing to visit an auctioneer each cycle. When a guest decides to visit an auctioneer, they select a random auctioneer from all available types (Dutch, English, or Vickrey) and move to a random position around that auctioneer to avoid clustering. Once at the auctioneer's location (within 6 units), guests can participate in auctions for items they are interested in.

The range restriction is enforced in the \textit{receiveCFP} reflex, which only allows guests to process auction proposals when they are within 6 units of their target auctioneer or have already reached the auctioneer location:
...
\begin{verbatim}
    reflex receiveCFP when: !empty(cfps) and hunger < hungerThreshold 
                       ...
                       and ((targetAuctioneer != nil 
                             and (location distance_to targetAuctioneer.location) < 6.0) 
                            or currentAction = "at auctioneer") {
        // Process CFP messages only when within range of an auctioneer
        ...
    }
\end{verbatim}

\begin{verbatim}
    // Global probability for guests to visit auctioneers
    float probability_go_to_auctioneer <- 0.05; // 5% chance per cycle

    reflex wander when: hunger < hungerThreshold and thirsty < thirstThreshold 
                       and onTheWayToShop = false and targetShop = nil 
                       and beingAttacked = false and goingToAuctioneer = false {
        ...
        if (flip(probability_go_to_auctioneer)) {
            ...
            list<agent> all_auctioneers <- list(Auctioneer) 
                                         + list(EnglishAuctioneer) 
                                         + list(VickreyAuctioneer);
            if (length(all_auctioneers) > 0) {
                targetAuctioneer <- one_of(all_auctioneers);
                // Calculate a random position around the auctioneer 
                // (circle with radius 3-5 units)
                int angle_degrees <- rnd(0, 360);
                float angle_radians <- angle_degrees * (3.14159 / 180.0);
                float distance <- rnd(3.0, 5.0);
                targetAuctioneerPosition <- targetAuctioneer.location 
                    + {distance * cos(angle_radians), 
                       distance * sin(angle_radians)};
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{imgs/user_wins_alcohol.png}
    \caption{Guest wins alcohol Dutch auction}
    \label{fig:alcohol_dutch}
\end{figure}

As it can be seen from the photo above, there has been an auction for alcohol which was won by Guest1
with price 1016.44. Two proposals were sent for that price.


\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\textwidth]{imgs/sugar_started.png}
    \caption{An auctioneer starts a Dutch auction for sugar}
    \label{fig:sugar_start}
\end{figure}

In figure \ref{fig:sugar_start} you will see a Dutch auction starting for sugar.
...
can three independent auctions simultaneously (alcohol, sugar, magic\_crystals) 
using vectorized state management and separate reflexes per item.
All auction state is stored in lists indexed by item (0=alcohol, 1=sugar, 2=magic\_crystals). This is done because
...
\begin{verbatim}
    list<string> interest_items <- ["alcohol", "sugar", "magic_crystals"];
\end{verbatim}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.4\textwidth]{imgs/three_same_time.png}
  \caption{Three auctions starting at the same time}
  \label{fig:three_same_time}
\end{figure}

As it can be seen from figure \ref{fig:three_same_time} we have three Dutch auctions for different products that are 
happening at the same time. They are also running together.


\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{imgs/not_interested.png}
  \caption{A guest is not interested into a product}
  \label{fig:not_interested}
\end{figure}

Figure \ref{fig:not_interested} shows that Guest7 is not interested into alcohol (at time 40.0).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{imgs/completed_while_running.png}
  \caption{Two auctions are completed while the other two are running}
  \label{fig:completed_while_running}
\end{figure}

In figure \ref{fig:completed_while_running} we can see that the auctioneer
had found offers for magic crystals and sugar while the auction for alcohol is still going on.
...
\subsection*{5.2 Code}
To implement the two new auction types we created two new agents, called \textbf{EnglishAuctioneer} and \textbf{VickreyAuctioneer}.
Both agents share similar structure and logic to the Dutch auctioneer but with differences in auction mechanics.
In code snippet provided in \ref{fig:vickrey_winner_selection}, we can see how the Vickrey auction is concluded. 
Since Vickrey auctions require sealed bids, all proposals are collected without revealing bid amounts to other bidders.
Once the bidding phase ends, the auctioneer identifies the highest and second-highest bids to determine the winner and the price they will pay.
As seen it the code snippet, the auctioneer enforces that the second-highest bid price is paid by the winner.
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/vickrey_code_snipper.png}
    \caption{Code snippet: Vickrey auction price determination process}
    \label{fig:vickrey_winner_selection}
\end{figure}
Unlike Dutch and English auction, Vickrey auctions are not iterative (i.e., single shot). This means that our logic for Vickrey auction is changed in a way that odd cycles determine the start of the auction and even cycles determine the end of it (as implemented by \ref{fig:vickrey_code_conclussion}).
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/vickrey_code_conclusion.png}
    \caption{Code snippet: Vickrey auction conclusion process}
    \label{fig:vickrey_code_conclussion}
\end{figure}
With English auctions, the auctioneer starts with a low initial price and bidders openly bid higher amounts. The code snippet in \ref{fig:english_highest_bid} depicts how the highest local bid is updated.
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/english_code_highest_bid_update.png}
    \caption{Code snippet: English auction highest bid update process}
    \label{fig:english_highest_bid}
\end{figure}

\subsection*{5.3 Demonstration and comparison of prices}
...
\subsubsection*{English Auction in action}
Figure \ref{fig:english_auction_in_action} demonstrates an English auction in progress, where bidders are actively raising their bids for the item on sale.
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{imgs/british_business.png}
    \caption{English auction in action}
    \label{fig:english_auction_in_action}
\end{figure}
\subsubsection*{Vickrey Auction in action}
Figure \ref{fig:vickrey_auction_in_action} demonstrates a Vickrey auction in progress, where conclusions of auctions are visible.
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{imgs/vickrey_business.png}
    \caption{Vickrey auction in action}
    \label{fig:vickrey_auction_in_action}
\end{figure}
\subsubsection*{Price Comparison}
For the comparison of all three auction types, we can achieve that by comparing the social welfare generated by each auction type.
To keep things simple, we express social welfare as the sum of two components: the utility gained by the winning bidder and the revenue earned by the auctioneer.
We hold that the utility gained by the bidder is the difference between their valuation of the item and the price they paid.
We don't calculate the sum of utilitiies (i.e., total social welfare), however we implemented two dynamic real-time graphs in GAMA that depict surplus of all bidders and auctioneer's revenues of all auction types.
Figure \ref{fig:social_welfare_comparison} shows the comparison the surpluses and revenues of all three auction types.
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{imgs/social_welfare_comparison.png}
    \caption{Social welfare comparison of auction types}
    \label{fig:social_welfare_comparison}
\end{figure}
From the graphs, we can observe that the Vickrey auction tends to generate higher bidder surpluses ($72\%$) compared to the Dutch and English auctions. 
This is because bidders in Vickrey auctions are incentivized to bid their true valuations, leading to more efficient outcomes.
The English auction shows moderate bidder surpluses, as the open bidding process allows bidders to adjust their bids based on competition.
The Dutch auction, on the other hand, often results in lower bidder surpluses due to its descending price mechanism, which may lead to quicker sales but at potentially lower prices.
This might also be due to the implementation fact that single-shot Vickrey auctions happen significantly more frequently than any other auction types combined.
Due to the same reasons, the auctioneer's revenue is highest in the Vickrey auction ($41\%$) and in the Dutch auction ($41\%$), and lowest in the English auction ($19\%$).
% --- SECTION 6 ---
